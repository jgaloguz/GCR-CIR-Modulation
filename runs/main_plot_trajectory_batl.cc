...
//--------------------------------------------------------------------------------
   ...
   GeoVector init_pos(0.15 * GSL_CONST_CGSM_ASTRONOMICAL_UNIT / unit_length_fluid, 0.0, 0.0);
   init_pos.Rotate(gv_nz, M_2PI * 0.75);
...
// Initial momentum
   ...
   double mom0 = Mom(MeV_kinetic_energy * SPC_CONST_CGSM_MEGA_ELECTRON_VOLT / unit_energy_particle, specie);
   GeoVector init_mom = mom0 * UnitVec(gv_nx-gv_ny);
...
//--------------------------------------------------------------------------------
   ...
   if (mpi_config->is_boss) {
      ...
      std::string trajectory_file = "output_" + cir_date + "/main_test_trajectory_" + cir_date + ".lines";
      trajectory->SetStart();
      trajectory->Integrate();
      trajectory->InterpretStatus();
      trajectory->PrintTrajectory(trajectory_file, false, 0x01 | 0x02 | 0x04 | 0x08);
      std::cout << "Trajectory outputted to " << trajectory_file << std::endl;
